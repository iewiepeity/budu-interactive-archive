...
type State = { anomaly: number; affection: number; badges: string[] };
type Archive = State & { disturb: (amount?: number) => void; addAffection: (amount: number) => void; unlock: (id: string) => void; reset: () => void; ready: boolean };
const initial: State = { anomaly: 0, affection: 0, badges: [] };
...
export function ArchiveProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const raw = localStorage.getItem("budu-archive-v1"); if (raw) try { setState(JSON.parse(raw)); } catch {} setReady(true); }, []);

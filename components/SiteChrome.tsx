...
export function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <header className="nav">
      ...
      <nav className="desktop-nav"><Link href="/author">作者</Link><Link href="/#works">作品</Link><Link href="/community">讀者社群</Link><button onClick={() => setOpen(true)} className="badge-button">徽章 {archive.badges.length}<i>/5</i></button></nav>

...
export function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <header className="nav">
      ...
      <nav className="desktop-nav"><Link href="/author">作者</Link><Link href="/#works">作品</Link><Link href="/community">讀者社群</Link><div className="archive-menu"><button>互動檔案 <span>⌄</span></button><div><Link href="/desk"><b>許哲維的書桌</b><small>調查失蹤前的工作現場</small></Link><Link href="/characters"><b>角色檔案</b><small>官方介紹與作者私評</small></Link><Link href="/quiz"><b>魔法傾向測驗</b><small>不保證正經的測驗結果</small></Link><Link href="/archive"><b>世界線存檔</b><small>進度、好感度與周目</small></Link></div></div><button onClick={() => setOpen(true)} className="badge-button">徽章 {archive.badges.length}<i>/5</i></button></nav>
